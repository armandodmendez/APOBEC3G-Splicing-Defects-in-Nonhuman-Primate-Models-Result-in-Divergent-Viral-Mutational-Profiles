...
def parse_args(args, iupac_dict):
    ...
    parser.add_argument(
        "bam",
        type=str,
        help="mapped file in bam format")
    parser.add_argument(
        "reference",
        type=str,
        help="reference sequence fro the bam file",
    )
    parser.add_argument(
        "mutationfrom",
        type=str.upper,
...
def read_bam(ref, bam_iter, current_read=None):
    if current_read is None:
        try:
            current_read = next(bam_iter)
        except StopIteration:
            return None, None, None, None

    name = current_read.query_name
    seq = current_read.query_sequence.upper()
    aligned_pairs = current_read.aligned_pairs
    ...
    for Q_pos, R_pos in aligned_pairs:
        if Q_pos is None:
            ...
            reconstructed_query.append(seq[Q_pos])
        else:
            reconstructed_ref.append(ref[R_pos])
            reconstructed_query.append(seq[Q_pos])
    ...
    try:
        ...
        next_read = None
    seq = "".join(reconstructed_query)
    ref = "".join(reconstructed_ref)
    return name, seq, ref, next_read
...
def loop_through_sequences(reference, bam_iter, args, iupac_dict, summaryfile, positionsfile):

    seqs = 1
    name, sequence, refseq, next_read = read_bam(reference, bam_iter)
    while next_read:
        ...
        seqs += 1
        name, sequence, refseq, next_read = read_bam(reference, bam_iter, next_read)
    primarysites, primaries, controlsites, controls = summarize_matches(
        refseq,
        sequence,
        args.begin,
...
if __name__ == "__main__":
    ...
    loop_through_sequences(ref, ba, args, iupac_dict, sf, pf)
