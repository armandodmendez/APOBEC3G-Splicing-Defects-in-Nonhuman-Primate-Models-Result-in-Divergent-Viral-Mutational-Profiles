#!/bin/bash
#$ -N pacbio-HIV-SIV-preprocessing-mapping.sh
#$ -cwd
#$ -pe smp 24
#$ -q all.q
...
############################
# RNA: with HIV/SIV subdirectories, mapped output, and samtools -F 2308
############################

cd /master/amendez/Experimental-ADM/HIV-SIV-Hypermutation-Assay-PacBio/split-barcode/RNA/trimmed || exit 1
...
for virus in HIV SIV; do
  ...
  for bam in *.trimmed.bam; do
    ...
    # Use -F 2308 to remove secondary, supplementary, and unmapped reads
